    ...
    if ($(e.target).closest('input, button, select').length === 0) {
        if ($('#listOfCar').find('button').length) {
            ...
            if ($('[name=firstName]').val() == '') {
                $('[name=firstName]').sendkeys('مهدی')//نام
            }
            if ($('[name=lastName]').val() == '') {
                $('[name=lastName]').sendkeys('مسیبی')//نام خانوادگی
            }
            if ($('[name=userName]').val() == '') {
                $('[name=userName]').sendkeys('0919591698')//شماره ملی
            }
            if ($('[name=password]').val() == '') {
                $('[name=password]').sendkeys('18')//شماره شناسنامه
            }
            if ($('[name=birthDate]').val() == '') {
                $('[name=birthDate]').sendkeys('1357/06/25')//تاریخ تولد
            }
