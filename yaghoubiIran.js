    ...
    if ($(e.target).closest('input, button, select').length === 0) {
        ...
        if ($('[name=loginForm]').length) {
            if ($('[name=firstName]').val() == '') {
                $('[name=firstName]').sendkeys('مجتبی') // name
            }
            if ($('[name=lastName]').val() == '') {
                $('[name=lastName]').sendkeys('یعقوبی خیبری') // family
            }
            if ($('[name=userName]').val() == '') {
                $('[name=userName]').sendkeys('0919944973')// shomare melli
            }
            if ($('[name=password]').val() == '') {
                $('[name=password]').sendkeys('26')// sehnasname
            }
            if ($('[name=birthDate]').val() == '') {
                $('[name=birthDate]').sendkeys('1365/01/01')// birth
            }
